...
def test(model, device, config, epochs=5, batch_size=1, save_cp=True, log_step=20, img_scale=0.5):
    ...
    test_dataset = Yolo_dataset(config.test_label, config, train=False)
    n_test = len(test_dataset)
    test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False,
                              num_workers=1, pin_memory=True, drop_last=True, collate_fn=val_collate)
...
#        model_input = [[cv2.resize(img, (544, 1024))] for img in images]
#        model_input = np.concatenate(model_input, axis=0)
#        model_input = model_input.transpose(0, 3, 1, 2)
#        model_input = torch.from_numpy(model_input).div(255.0)
#        model_input = model_input.to(device)
#        output = model(model_input)
#        print(utils.post_processing(1, 0.4, 0.6, output))
#        break
#
#    return
    ...
    # batch of 1 sample image
    img = torch.rand(1, 3, 544, 1024).to(device)
    ...
    with torch.no_grad():
        for images, targets in tqdm(test_loader):
            ...
            model_input = [[cv2.resize(img, (test_width, test_height))] for img in images]
            model_input = np.concatenate(model_input, axis=0)
            model_input = model_input.transpose(0, 3, 1, 2)
            model_input = torch.from_numpy(model_input).div(255.0)
            model_input = model_input.to(device)
